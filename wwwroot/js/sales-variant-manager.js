(function(){
  ...
  window.SalesVariantManager={
    getData(){
      return {variantTypes:state.types.map(t=>({name:t.name,selectedPreset:t.selectedPreset,options:t.options.map(o=>({id:o.id,name:o.name,description:o.desc}))})),rows:JSON.parse(JSON.stringify(state.rows)),sizeTemplate:root.querySelector('#sizeTemplate').value};
    },
    setData(data){
      if(!data) return;
      ...
      if(data.rows){
        const primaryByName={},addonByName={};
        (state.types[0]?.options||[]).forEach(o=>primaryByName[o.name]=o.id);
        (state.types[1]?.options||[]).forEach(o=>addonByName[o.name]=o.id);
        state.rows={};
        Object.entries(data.rows).forEach(([key,row])=>{
          const parts=String(key).split('__');
          let newKey=key;
          if(primaryByName[parts[0]]){
            const pId=primaryByName[parts[0]];
            const aName=parts[1];
            const aId=(aName!=null&&aName!=='')?addonByName[aName]:null;
            newKey=aId?`${pId}__${aId}`:`${pId}__empty-addon`;
          }
          state.rows[newKey]=row;
        });
      }
      renderTypes();renderMatrix();
    }
